...
*/
func NewServer(addr, network string, handler http.Handler) (srv *endlessServer) { // {{{
...
func ListenAndServeTcp(addr, network string, tcpServer TcpServer) error { // {{{
	server := NewServer(addr, network, nil)
...
func (srv *endlessServer) setTcpServer(tcpServer TcpServer) { // {{{
...
func (srv *endlessServer) Serve() (err error) { // {{{
	...
	if nil != srv.tcpServer {
		err = srv.tcpServer.Serve(srv.EndlessListener)
	} else {
...
func (srv *endlessServer) gracefulStop() { // {{{
	if grpcServer, ok := srv.tcpServer.(*grpc.Server); ok {
...
func (srv *endlessServer) hammerTime(d time.Duration) { // {{{
	defer func() {
		...
		if r := recover(); r != nil {
			if err, ok := r.(string); ok && strings.Contains(err, "negative WaitGroup counter") {
				log.Println("WaitGroup at 0", r)

				// 等待其他并发处理结束
				time.Sleep(10e6)

				atomic.AddInt32(&closeFlag, 1)

				// 尝试修正 negative WaitGroup counter 错误, 让 wg.wait() 可以继续运行
				d := int(atomic.LoadInt32(&closeFlag))
				log.Println("try to fix negative value: ", d)
				srv.wg.Add(d)
			}
		}
	}()
...
func (el *endlessListener) Accept() (c net.Conn, err error) { // {{{
	...
	defer func() {
		if r := recover(); r != nil {
			if err, ok := r.(string); ok && strings.Contains(err, "negative WaitGroup counter") {
				log.Println("negative WaitGroup counter When Accept() ")
				atomic.AddInt32(&closeFlag, -1)
			}
		}
	}()
	...
	if atomic.LoadInt32(&closeFlag) == 0 { // closeFlag >0 时，说明 wg counter 已为负，已处于 shutdown 状态，不再执行 Add()
		el.server.wg.Add(1)
	}
...
func (w endlessConn) Close() error { // {{{
	...
	defer func() {
		if r := recover(); r != nil {
			if err, ok := r.(string); ok && strings.Contains(err, "negative WaitGroup counter") {
				log.Println("negative WaitGroup counter When Close() ")
				atomic.AddInt32(&closeFlag, 1)
			}
		}
	}()
	...
	err := w.Conn.Close()
	if err == nil && atomic.LoadInt32(&closeFlag) == 0 { // closeFlag >0 时，说明 wg counter 已为负，不再执行 Done()
		w.server.wg.Done()
	}
	return err
} // }}}
