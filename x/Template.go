...
var (
	...
	TemplateFuncs  = template.FuncMap{
		...
		"asint":      AsInt,
		"asfloat":    AsFloat,
		"default": func(def, v interface{}) interface{} { //模版变量是最后一个参数
			if v == nil {
				return def
			}
...
func (this *Template) loadTemplateData(template_file string) (string, interface{}, error) { // {{{
	...
	var err error
	if templateUseEmbed {
		buffer, err = embedTemplates.ReadFile(filepath.Join(embedTemplatePath, template_file))
	} else {
		template_root := Conf.Get("template", "template_root").String(TemplateRoot)

		if "" != template_root && !filepath.IsAbs(template_root) {
			template_root = filepath.Join(AppRoot, template_root)
		}

		f, err := os.Open(filepath.Join(template_root, template_file))
		if err != nil {
			return "", nil, err
		}
		defer f.Close()
	...
	if len(matches) > 0 {
		...
		for _, m := range matches {
			if len(m) == 4 {
				...
				recursion_limit := Conf.Get("recursion_limit").Int(3)
